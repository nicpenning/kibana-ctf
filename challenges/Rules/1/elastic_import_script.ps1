...
function challenge {
    ...
    $importRuleResponse = Invoke-RestMethod -Method POST -Uri "$Kibana_URL/s/kibana-ctf/api/detection_engine/rules/_import" -Headers @{"kbn-xsrf"="true"; "Authorization"="$kibanaAuth"} -Form @{file = Get-Item $ruleFilePath} -AllowUnencryptedAuthentication -SkipCertificateCheck
